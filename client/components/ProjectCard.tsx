...
export const ProjectCard = ({
  ...
  // Springs optimized for snappy, non-laggy response
  ...
  // Increased tilt range for better effect since content is static
  ...
  return (
    // 1. ROOT CONTAINER: Pure 2D, handles all events.
    <div 
      ref={rootRef}
    ...
    >
      {/* 2. 3D BACKGROUND LAYER (Tilt Effect) - Pointer events NONE */}
      <motion.div 
        style={{ 
        ...
        className={`absolute inset-0 z-0 bg-[#0a0f0a] border-2 rounded-lg transition-colors duration-300 pointer-events-none shadow-[0_0_15px_rgba(0,255,65,0.05)] ${
      ...
      >
        ...
        {/* Decorative Glow */}
        <div className={`absolute inset-0 bg-primary/0 transition-all duration-700 ${isHovered ? 'bg-primary/[0.03]' : ''}`} />
      </motion.div>

      {/* 3. CONTENT LAYER (Static 2D) - Interactive and Stable */}
      <div className={`relative z-10 ${isDossier ? 'p-4 sm:p-6' : 'p-6 sm:p-8'}`}>
        <div className={`flex flex-col ${isDossier ? 'xl:flex-row' : 'lg:flex-row'} gap-4 sm:gap-8`}>
          
          {/* Image Container */}
          <div className={`${isDossier ? 'xl:w-1/4' : 'lg:w-1/3'} shrink-0`}>
