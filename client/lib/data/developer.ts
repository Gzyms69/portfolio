...
export const devProjects: Project[] = [
  {
    ...
    pl: { 
      description: "**Architektura & UI:** Ekosystem monorepo (**Turborepo**) z własną biblioteką komponentów (Framer Motion, Tailwind). Nowoczesny frontend (**Next.js App Router**, 98+/100 Lighthouse) i zautomatyzowane SEO (OpenGraph). **Symulacja Backend & Stan:** Wdrożenie bezstanowego środowiska demo e-commerce (panel admina, sklep, profil klienta) opartego całkowicie na zarządzaniu sesją i ciasteczkami (cookies), pozwalającego na bezpieczne testowanie platformy. **Open Source & DevOps:** Fork repozytorium `google-maps-scraper` (**Playwright**) – analiza i merge patchy społeczności (Pull Requests). Wdrożenie endpointów telemetrycznych zapobiegających 'cold start', ochrona domeny (**Cloudflare**) oraz zautomatyzowane debugowanie logów przy użyciu przeglądarki headless (**Puppeteer**) i środowisk LLM z serwerami MCP.", 
